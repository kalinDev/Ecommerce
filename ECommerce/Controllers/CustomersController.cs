...
namespace ECommerce.Controllers {
  ...
  public class CustomersController : ControllerBase {
    ...
    public async Task<ActionResult<IEnumerable<Customer>>> GetAsync() {
      return Ok(
          _mapper.Map<IEnumerable<CustomerDTO>>(await _repository.FindAsync()));
    }
    ...
    PostAsync([FromBody] PostCustomerDTO postCustomerDTO) {
      var customer = _mapper.Map<Customer>(postCustomerDTO);
      var customerExists =
          (await _repository.SearchAsync(c => c.Email == customer.Email)).Any();
    ...
    [HttpPut("{id:Guid}")]
    public async Task<ActionResult>
    PutAsync(Guid id, [FromBody] CustomerDTO customerDTO) {
      var customer = _mapper.Map<Customer>(customerDTO);
      if (id != customer.Id)
        return BadRequest();

      var customerExists = (await _repository.SearchAsync(
                                c => c.Email == customer.Email && c.Id != id))
                               .Any();
      if (customerExists) return Conflict("The email is already in use");
    ...
    CustomerExists(Guid id) => await _repository.AnyAsync(id);
  }
}
